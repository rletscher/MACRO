  ...
  subroutine marbl_interior_tendency_share_export_variables(&
    ...
    if (ciso_on) then
      ...
      interior_tendency_share%DOCtot_remin_fields(:) = &
	   ...
	   dissolved_organic_matter%lipid_remin(:) + dissolved_organic_matter%DOCr_remin(:)
      interior_tendency_share%DOCtot_loc_fields(:) = &
           tracer_local(marbl_tracer_indices%poly_ind,:)  + &
           tracer_local(marbl_tracer_indices%lipid_ind,:) + tracer_local(marbl_tracer_indices%DOCr_ind,:)
    end if
