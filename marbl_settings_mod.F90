module marbl_settings_mod
  ...
  ! Remin Ratios, macromolecular DOM
  real(r8), parameter :: &
       parm_Remin_poly_C_O2   = 0.44_r8,            & ! polysaccharide carbon:oxygen for oxic remin
       parm_Den_poly_C_NO3    = 0.56_r8,            & ! polysaccharide carbon:nitrate for denitrif
       parm_Remin_poly_TA_P   = -1.0_r8,            & ! polysaccharide TAlk:phosphorus change for oxic remin
       parm_Den_poly_TA_P     = 0.8_r8,             & ! polysaccharide TAlk:phosphorus change for denitrif
       parm_Remin_lipid_C_O2  = 0.65_r8,            & ! lipid carbon:oxygen for oxic remin
       parm_Den_lipid_C_NO3   = 0.82_r8,            & ! lipid carbon:nitrate for denitrif
       parm_Remin_lipid_TA_P  = -1.0_r8,            & ! lipid TAlk:phosphorus change for oxic remin
       parm_Den_lipid_TA_P    = 9.67_r8,            & ! lipid TAlk:phosphorus change for denitrif
       parm_Remin_apoly_C_O2  = 1.14_r8,            & ! amino_polysaccharide carbon:oxygen for oxic remin
       parm_Den_apoly_C_NO3   = 1.43_r8,            & ! amino_polysaccharide carbon:nitrate for denitrif
       parm_Remin_apoly_TA_P  = 1.29_r8,            & ! amino_polysaccharide TAlk:phosphorus change for oxic remin
       parm_Den_apoly_TA_P    = 7.84_r8,            & ! amino_polysaccharide TAlk:phosphorus change for denitrif
       parm_Remin_prot_C_O2   = 1.2_r8,             & ! protein carbon:oxygen for oxic remin
       parm_Den_prot_C_NO3    = 1.5_r8,             & ! protein carbon:nitrate for denitrif
       parm_Remin_prot_TA_P   = 2.24_r8,            & ! protein TAlk:phosphorus change for oxic remin
       parm_Den_prot_TA_P     = 8.39_r8,            & ! protein TAlk:phosphorus change for denitrif
       parm_Remin_DOP_C_O2    = 0.76_r8,            & ! sl DOP carbon:oxygen for oxic remin
       parm_Den_DOP_C_NO3     = 0.95_r8,            & ! sl DOP carbon:nitrate for denitrif
       parm_Remin_DOP_TA_P    = -0.81_r8,           & ! sl DOP TAlk:phosphorus for change oxic remin
       parm_Den_DOP_TA_P      = 6.27_r8,            & ! sl DOP TAlk:phosphorus for change denitrif
       parm_Remin_rDOM_C_O2   = 0.92_r8,            & ! refractory DOM carbon:oxygen for oxic remin
       parm_Den_rDOM_C_NO3    = 1.15_r8,            & ! refractory DOM carbon:nitrate for denitrif
       parm_Remin_rDOM_TA_P   = 60.9_r8,            & ! refractory DOM TAlk:phosphorus change for oxic remin
       parm_Den_rDOM_TA_P     = 1281.3_r8             ! refractory DOM TAlk:phosphorus change for denitrif
  ...
  real(r8), parameter :: &
      ...
      f_toDON               = 0.7_r8,   & ! fraction DON relative to DOC
  ...
  real(r8), parameter :: &
      ...
      Q_prot        = 1.0_r8 / 3.61_r8,   & !N/C ratio (mmol/mmol) of protein
      Q_apoly       = 1.0_r8 / 8.0_r8,    & !N/C ratio (mmol/mmol) of amino polysaccharides
  ...
  real(r8), parameter :: &
       poly_reminR_light        = (c1/(365.0_r8*27.0_r8)) * dps, & ! remin rate for polysaccharides, 1/27yr
       lipid_reminR_light       = (c1/(365.0_r8*1.0_r8))  * dps, & ! remin rate for lipids, 1/1yr
       amino_poly_reminR_light  = (c1/(365.0_r8*60.0_r8)) * dps, & ! remin rate for amino polysaccharides, 1/60yr
       protein_reminR_light     = (c1/(365.0_r8*3.0_r8))  * dps, & ! remin rate for proteins, 1/3yr
       DOP_reminR_light         = (c1/(365.0_r8*60.0_r8)) * dps, & ! remin rate for semi-labile DOP, 1/60yr
       poly_reminR_dark         = (c1/(365.0_r8*9.0_r8))  * dps, & ! remin rate in the dark, 1/9yr
       lipid_reminR_dark        = (c1/(365.0_r8*1.0_r8))  * dps, & ! remin rate in the dark, 1/1yr
       amino_poly_reminR_dark   = (c1/(365.0_r8*20.0_r8)) * dps, & ! remin rate in the dark, 1/20yr
       protein_reminR_dark      = (c1/(365.0_r8*3.0_r8))  * dps, & ! remin rate in the dark, 1/3.0yr
...
contains
  ...
  subroutine marbl_settings_define_PFT_derived_types(this, marbl_status_log)
    ...
    do n=1,autotroph_cnt
      ...
      write(sname, "(2A)") trim(prefix), 'f_poly'
      lname    = 'routing of biomass to polysaccharides'
      units    = 'unitless'
      datatype = 'real'
      rptr     => autotroph_settings(n)%f_poly
      call this%add_var(sname, lname, units, datatype, category,     &
                        marbl_status_log, rptr=rptr,                 &
                        nondefault_required=(PFT_defaults .eq. 'user-specified'))
      call check_and_log_add_var_error(marbl_status_log, sname, subname, labort_marbl_loc)

      write(sname, "(2A)") trim(prefix), 'f_lipid'
      lname    = 'routing of biomass to lipids'
      units    = 'unitless'
      datatype = 'real'
      rptr     => autotroph_settings(n)%f_lipid
      call this%add_var(sname, lname, units, datatype, category,     &
                        marbl_status_log, rptr=rptr,                 &
                        nondefault_required=(PFT_defaults .eq. 'user-specified'))
      call check_and_log_add_var_error(marbl_status_log, sname, subname, labort_marbl_loc)

      write(sname, "(2A)") trim(prefix), 'f_amino_poly'
      lname    = 'routing of biomass to amino polysaccharides'
      units    = 'unitless'
      datatype = 'real'
      rptr     => autotroph_settings(n)%f_amino_poly
      call this%add_var(sname, lname, units, datatype, category,     &
                        marbl_status_log, rptr=rptr,                 &
                        nondefault_required=(PFT_defaults .eq. 'user-specified'))
      call check_and_log_add_var_error(marbl_status_log, sname, subname, labort_marbl_loc)

      write(sname, "(2A)") trim(prefix), 'f_protein'
      lname    = 'routing of biomass to proteins'
      units    = 'unitless'
      datatype = 'real'
      rptr     => autotroph_settings(n)%f_protein
      call this%add_var(sname, lname, units, datatype, category,     &
                        marbl_status_log, rptr=rptr,                 &
                        nondefault_required=(PFT_defaults .eq. 'user-specified'))
      call check_and_log_add_var_error(marbl_status_log, sname, subname, labort_marbl_loc)
    ...
    do n=1, zooplankton_cnt
      ...
      units    = 'unitless'
      datatype = 'real'
      rptr     => zooplankton_settings(n)%f_poly
      call this%add_var(sname, lname, units, datatype, category,       &
                        marbl_status_log, rptr=rptr,                 &
                        nondefault_required=(PFT_defaults .eq. 'user-specified'))
      call check_and_log_add_var_error(marbl_status_log, sname, subname, labort_marbl_loc)
      ...
      units    = 'unitless'
      datatype = 'real'
      rptr     => zooplankton_settings(n)%f_lipid
      call this%add_var(sname, lname, units, datatype, category,       &
                        marbl_status_log, rptr=rptr,                 &
                        nondefault_required=(PFT_defaults .eq. 'user-specified'))
      call check_and_log_add_var_error(marbl_status_log, sname, subname, labort_marbl_loc)
      ...
      units    = 'unitless'
      datatype = 'real'
      rptr     => zooplankton_settings(n)%f_amino_poly
      call this%add_var(sname, lname, units, datatype, category,       &
                        marbl_status_log, rptr=rptr,                 &
                        nondefault_required=(PFT_defaults .eq. 'user-specified'))
      call check_and_log_add_var_error(marbl_status_log, sname, subname, labort_marbl_loc)
      ...
      units    = 'unitless'
      datatype = 'real'
      rptr     => zooplankton_settings(n)%f_protein
      call this%add_var(sname, lname, units, datatype, category,       &
                        marbl_status_log, rptr=rptr,                 &
                        nondefault_required=(PFT_defaults .eq. 'user-specified'))
      call check_and_log_add_var_error(marbl_status_log, sname, subname, labort_marbl_loc)
