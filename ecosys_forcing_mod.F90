module ecosys_forcing_mod
  ...
  use ecosys_tracers_and_saved_state_mod, only : no3_ind, po4_ind, amino_poly_ind, protein_ind, donr_ind, dop_ind, dopr_ind
  use ecosys_tracers_and_saved_state_mod, only : sio3_ind, fe_ind, poly_ind, lipid_ind, docr_ind, do13ctot_ind, do14ctot_ind
    ...
    do n = 1, size(riv_flux_components)
      select case (trim(riv_flux_components(n)))
        ...
        case ('don')
          ...
          lhas_riv_flux(amino_poly_ind) = .true.
          lhas_riv_flux(protein_ind)    = .true.
        ...
        case ('doc')
          ...
          lhas_riv_flux(poly_ind)  = .true.
          lhas_riv_flux(lipid_ind) = .true.
...
!!!     end associate
  ...
  subroutine ecosys_forcing_comp_stf_riv(ciso_on, stf_riv, iblock)
    ...
    if (riv_flux_ind > 0) then
      processed_field_cnt   = processed_field_cnt + 1
      stf_riv(:,:,amino_poly_ind)  = (c1 - DONriv_refract) * 0.6_r8 * riv_flux_forcing_fields(riv_flux_ind)%field_0d(:,:,iblock)
      stf_riv(:,:,protein_ind)     = (c1 - DONriv_refract) * 0.4_r8 * riv_flux_forcing_fields(riv_flux_ind)%field_0d(:,:,iblock)
    ...
    if (riv_flux_ind > 0) then
      processed_field_cnt   = processed_field_cnt + 1
      stf_riv(:,:,poly_ind)   = (c1 - DOCriv_refract) * 0.9_r8 * riv_flux_forcing_fields(riv_flux_ind)%field_0d(:,:,iblock)
      stf_riv(:,:,lipid_ind)  = (c1 - DOCriv_refract) * 0.1_r8 * riv_flux_forcing_fields(riv_flux_ind)%field_0d(:,:,iblock)
