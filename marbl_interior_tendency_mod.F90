module marbl_interior_tendency_mod
  ...
  use marbl_settings_mod, only : Q_apoly
  use marbl_settings_mod, only : Q_prot
  ...
  use marbl_settings_mod, only : parm_Remin_poly_C_O2
  use marbl_settings_mod, only : parm_Remin_lipid_C_O2
  use marbl_settings_mod, only : parm_Remin_rDOM_C_O2
  use marbl_settings_mod, only : parm_Den_poly_C_NO3
  use marbl_settings_mod, only : parm_Den_lipid_C_NO3
  use marbl_settings_mod, only : parm_Den_rDOM_C_NO3
...
contains
  ...
  subroutine marbl_interior_tendency_compute( &
    ...
    associate(                                                      &
         ...
         poly_ind          => marbl_tracer_indices%poly_ind,        &
         lipid_ind         => marbl_tracer_indices%lipid_ind,       &
         amino_poly_ind    => marbl_tracer_indices%amino_poly_ind,  &
         protein_ind       => marbl_tracer_indices%protein_ind,     &
    ...
    call compute_denitrif(km, marbl_tracer_indices, tracer_local(:, :), &
         dissolved_organic_matter%poly_remin(:),  &
         dissolved_organic_matter%lipid_remin(:), &
  ...
  subroutine compute_autotroph_loss(km, Tfunc, autotroph_derived_terms)
    ...
    associate(                                                         &
         ...
         auto_loss_poly  => autotroph_derived_terms%auto_loss_poly(:, :),  & ! output
         auto_loss_lipid => autotroph_derived_terms%auto_loss_lipid(:, :), & ! output
         auto_loss_amino_poly => autotroph_derived_terms%auto_loss_amino_poly(:, :), & ! output
         auto_loss_protein    => autotroph_derived_terms%auto_loss_protein(:, :),    & ! output
      ...
      do auto_ind = 1, autotroph_cnt
        ...
        auto_loss_poly(auto_ind,:)       = auto_loss_doc(auto_ind,:) * autotroph_settings(auto_ind)%f_poly
        auto_loss_lipid(auto_ind,:)      = auto_loss_doc(auto_ind,:) * autotroph_settings(auto_ind)%f_lipid
        auto_loss_amino_poly(auto_ind,:) = auto_loss_doc(auto_ind,:) * autotroph_settings(auto_ind)%f_amino_poly
        auto_loss_protein(auto_ind,:)    = auto_loss_doc(auto_ind,:) * autotroph_settings(auto_ind)%f_protein
  ...
  subroutine compute_grazing(km, Tfunc, zooplankton_local, zooplankton_derived_terms, autotroph_derived_terms)
    ...
    associate(                                                           &
         ...
         auto_graze_poly  => autotroph_derived_terms%auto_graze_poly(:,:),   & ! output
         auto_graze_lipid => autotroph_derived_terms%auto_graze_lipid(:,:),  & ! output
         auto_graze_amino_poly => autotroph_derived_terms%auto_graze_amino_poly(:,:),  & ! output
         auto_graze_protein    => autotroph_derived_terms%auto_graze_protein(:,:),     & ! output
         ...
         zoo_graze_poly   => zooplankton_derived_terms%zoo_graze_poly(:,:),  & ! output
         zoo_graze_lipid  => zooplankton_derived_terms%zoo_graze_lipid(:,:), & ! output
         zoo_graze_amino_poly  => zooplankton_derived_terms%zoo_graze_amino_poly(:,:), & ! output
         zoo_graze_protein     => zooplankton_derived_terms%zoo_graze_protein(:,:),    & ! output
      ...
      auto_graze_poly(:,:)       = c0 ! autotroph grazing losses routed to poly
      auto_graze_lipid(:,:)      = c0 ! autotroph grazing losses routed to lipid
      auto_graze_amino_poly(:,:) = c0 ! autotroph grazing losses routed to amino_poly
      auto_graze_protein(:,:)    = c0 ! autotroph grazing losses routed to protein
      ...
      zoo_graze_poly(:,:)       = c0 ! zooplankton grazing losses routed to poly
      zoo_graze_lipid(:,:)      = c0 ! zooplankton grazing losses routed to lipid
      zoo_graze_amino_poly(:,:) = c0 ! zooplankton grazing losses routed to amino_poly
      zoo_graze_protein(:,:)    = c0 ! zooplankton grazing losses routed to protein
        ...
        do pred_ind = 1, zooplankton_cnt
          ...
          do prey_ind = 1, max_grazer_prey_cnt
            ...
            do auto_ind2 = 1, grazing_relationship_settings(prey_ind, pred_ind)%auto_ind_cnt
              ...
              auto_graze_poly(auto_ind,k)       = auto_graze_doc(auto_ind,k) * autotroph_settings(auto_ind)%f_poly
              auto_graze_lipid(auto_ind,k)      = auto_graze_doc(auto_ind,k) * autotroph_settings(auto_ind)%f_lipid
              auto_graze_amino_poly(auto_ind,k) = auto_graze_doc(auto_ind,k) * autotroph_settings(auto_ind)%f_amino_poly
              auto_graze_protein(auto_ind,k)    = auto_graze_doc(auto_ind,k) * autotroph_settings(auto_ind)%f_protein
            ...
            do zoo_ind2 = 1, grazing_relationship_settings(prey_ind, pred_ind)%zoo_ind_cnt
              ...
              zoo_graze_doc(zoo_ind,k) = zoo_graze_doc(zoo_ind,k) &
                                       + grazing_relationship_settings(prey_ind, pred_ind)%graze_doc * work2
              zoo_graze_poly(zoo_ind,k)       = zoo_graze_doc(zoo_ind,k) * zooplankton_settings(zoo_ind)%f_poly
              zoo_graze_lipid(zoo_ind,k)       = zoo_graze_doc(zoo_ind,k) * zooplankton_settings(zoo_ind)%f_lipid
              zoo_graze_amino_poly(zoo_ind,k) = zoo_graze_doc(zoo_ind,k) * zooplankton_settings(zoo_ind)%f_amino_poly
              zoo_graze_protein(zoo_ind,k)    = zoo_graze_doc(zoo_ind,k) * zooplankton_settings(zoo_ind)%f_protein
  ...
  subroutine compute_routing(km, zooplankton_derived_terms, autotroph_derived_terms)
    ...
    associate(                                                           &
         ...
         zoo_loss_poly     => zooplankton_derived_terms%zoo_loss_poly(:,:),   & ! output
         zoo_loss_lipid    => zooplankton_derived_terms%zoo_loss_lipid(:,:),  & ! output
         zoo_loss_amino_poly    => zooplankton_derived_terms%zoo_loss_amino_poly(:,:),  & ! output
         zoo_loss_protein       => zooplankton_derived_terms%zoo_loss_protein(:,:),     & ! output
      ...
      do k=1,km
        ...
        do zoo_ind = 1, zooplankton_cnt
          ...
          zoo_loss_poly(zoo_ind,k)       = zoo_loss_doc(zoo_ind,k) * zooplankton_settings(zoo_ind)%f_poly
          zoo_loss_lipid(zoo_ind,k)      = zoo_loss_doc(zoo_ind,k) * zooplankton_settings(zoo_ind)%f_lipid
          zoo_loss_amino_poly(zoo_ind,k) = zoo_loss_doc(zoo_ind,k) * zooplankton_settings(zoo_ind)%f_amino_poly
          zoo_loss_protein(zoo_ind,k)    = zoo_loss_doc(zoo_ind,k) * zooplankton_settings(zoo_ind)%f_protein
  ...
  subroutine compute_dissolved_organic_matter (km, marbl_tracer_indices, &
    ...
    use marbl_settings_mod, only : Q
    use marbl_settings_mod, only : poly_reminR_light
    use marbl_settings_mod, only : lipid_reminR_light
    use marbl_settings_mod, only : amino_poly_reminR_light
    use marbl_settings_mod, only : protein_reminR_light
    use marbl_settings_mod, only : DOP_reminR_light
    use marbl_settings_mod, only : poly_reminR_dark
    use marbl_settings_mod, only : lipid_reminR_dark
    use marbl_settings_mod, only : amino_poly_reminR_dark
    use marbl_settings_mod, only : protein_reminR_dark
    ...
    real(r8) :: work
    real(r8) :: poly_reminR       ! remineralization rate (1/sec)
    real(r8) :: lipid_reminR      ! remineralization rate (1/sec)
    real(r8) :: amino_poly_reminR ! remineralization rate (1/sec)
    real(r8) :: protein_reminR    ! remineralization rate (1/sec)
    ...
    associate(                                                           &
         poly_loc        => tracer_local(marbl_tracer_indices%poly_ind,:),        &
         lipid_loc       => tracer_local(marbl_tracer_indices%lipid_ind,:),       &
         amino_poly_loc  => tracer_local(marbl_tracer_indices%amino_poly_ind,:),  &
         protein_loc     => tracer_local(marbl_tracer_indices%protein_ind,:),     &
         DOP_loc         => tracer_local(marbl_tracer_indices%dop_ind,:),  &
         ...
         auto_loss_poly    => autotroph_derived_terms%auto_loss_poly(:,:),   & ! input
         auto_loss_lipid   => autotroph_derived_terms%auto_loss_lipid(:,:),  & ! input
         auto_loss_amino_poly   => autotroph_derived_terms%auto_loss_amino_poly(:,:),  & ! input
         auto_loss_protein      => autotroph_derived_terms%auto_loss_protein(:,:),     & ! input
         auto_graze_doc  => autotroph_derived_terms%auto_graze_doc(:,:),  & ! input
         auto_graze_poly   => autotroph_derived_terms%auto_graze_poly(:,:),   & ! input
         auto_graze_lipid  => autotroph_derived_terms%auto_graze_lipid(:,:),  & ! input
         auto_graze_amino_poly  => autotroph_derived_terms%auto_graze_amino_poly(:,:),  & ! input
         auto_graze_protein     => autotroph_derived_terms%auto_graze_protein(:,:),     & ! input
  
         zoo_loss_doc    => zooplankton_derived_terms%zoo_loss_doc(:,:),  & ! input
         zoo_loss_poly    => zooplankton_derived_terms%zoo_loss_poly(:,:),   & ! input
         zoo_loss_lipid   => zooplankton_derived_terms%zoo_loss_lipid(:,:),  & ! input
         zoo_loss_amino_poly   => zooplankton_derived_terms%zoo_loss_amino_poly(:,:),  & ! input
         zoo_loss_protein      => zooplankton_derived_terms%zoo_loss_protein(:,:),     & ! input
         zoo_graze_doc   => zooplankton_derived_terms%zoo_graze_doc(:,:), & ! input
         zoo_graze_poly   => zooplankton_derived_terms%zoo_graze_poly(:,:),   & ! input
         zoo_graze_lipid  => zooplankton_derived_terms%zoo_graze_lipid(:,:),  & ! input
         zoo_graze_amino_poly  => zooplankton_derived_terms%zoo_graze_amino_poly(:,:),  & ! input
         zoo_graze_protein     => zooplankton_derived_terms%zoo_graze_protein(:,:),     & ! input

         DOC_prod        => dissolved_organic_matter%DOC_prod(:),   & ! output production of DOC (mmol C/m^3/sec)
         poly_prod       => dissolved_organic_matter%poly_prod(:),  & ! output production of poly
         lipid_prod      => dissolved_organic_matter%lipid_prod(:), & ! output production of lipid
         poly_remin      => dissolved_organic_matter%poly_remin(:), & ! output remineralization of poly
         lipid_remin     => dissolved_organic_matter%lipid_remin(:), & ! output remineralization of lipid
         DOCr_remin      => dissolved_organic_matter%DOCr_remin(:), & ! output remineralization of DOCr
         amino_poly_prod => dissolved_organic_matter%amino_poly_prod(:),   & ! output production of amino_poly (mmol N/m^3/sec)
         protein_prod    => dissolved_organic_matter%protein_prod(:), & ! output production of protein
         amino_poly_remin => dissolved_organic_matter%amino_poly_remin(:),  & ! output remineralization of amino_poly
         protein_remin   => dissolved_organic_matter%protein_remin(:), & ! output remineralization of protein
      ...
      poly_prod(:)        = sum(zoo_loss_poly(:,:), dim=1)    + sum(auto_loss_poly(:,:), dim=1) &
                          + sum(auto_graze_poly(:,:), dim=1)  + sum(zoo_graze_poly(:,:), dim=1)
      lipid_prod(:)       = sum(zoo_loss_lipid(:,:), dim=1)   + sum(auto_loss_lipid(:,:), dim=1) &
                          + sum(auto_graze_lipid(:,:), dim=1) + sum(zoo_graze_lipid(:,:), dim=1)
      amino_poly_prod(:)  = Q * f_toDON * (sum(zoo_loss_amino_poly(:,:), dim=1)   + sum(auto_loss_amino_poly(:,:), dim=1) &
                          + sum(auto_graze_amino_poly(:,:), dim=1) + sum(zoo_graze_amino_poly(:,:), dim=1))
      protein_prod(:)     = Q * f_toDON * (sum(zoo_loss_protein(:,:), dim=1)       + sum(auto_loss_protein(:,:), dim=1) &
                          + sum(auto_graze_protein(:,:), dim=1)    + sum(zoo_graze_protein(:,:), dim=1))
!      amino_poly_prod(:) = Q_apoly * DOC_prod(:) * f_toDON
!      prot_prod(:) = Q_prot * DOC_prod(:) * f_toDON
...
! FIXME
        do subcol_ind = 1, PAR_nsubcols
          if (PAR_col_frac(subcol_ind) > c0) then
            if (PAR_avg(k,subcol_ind) > 1.0_r8) then
              poly_reminR       = poly_reminR + PAR_col_frac(subcol_ind) * poly_reminR_light
              lipid_reminR      = lipid_reminR + PAR_col_frac(subcol_ind) * lipid_reminR_light
              amino_poly_reminR = amino_poly_reminR + PAR_col_frac(subcol_ind) * amino_poly_reminR_light
              protein_reminR    = protein_reminR + PAR_col_frac(subcol_ind) * protein_reminR_light
              DOP_reminR        = DOP_reminR + PAR_col_frac(subcol_ind) * DOP_reminR_light
            else
              poly_reminR       = poly_reminR + PAR_col_frac(subcol_ind) * poly_reminR_dark
              lipid_reminR      = lipid_reminR + PAR_col_frac(subcol_ind) * lipid_reminR_dark
              amino_poly_reminR = amino_poly_reminR + PAR_col_frac(subcol_ind) * amino_poly_reminR_dark
              protein_reminR    = protein_reminR + PAR_col_frac(subcol_ind) * protein_reminR_dark
        ...
        poly_remin(k)        = poly_loc(k)        * poly_reminR
        lipid_remin(k)       = lipid_loc(k)       * lipid_reminR
        amino_poly_remin(k)  = amino_poly_loc(k)  * amino_poly_reminR
        protein_remin(k)     = protein_loc(k)     * protein_reminR
...
!        POC%hflux_out(k) = c0
  ...
  subroutine compute_denitrif(km, marbl_tracer_indices, tracer_local, poly_remin, DOCr_remin,   &
       lipid_remin, POC_remin, other_remin, sed_denitrif, denitrif)
    ...
    type(marbl_tracer_index_type), intent(in)    :: marbl_tracer_indices
    real(r8),                      intent(in)    :: tracer_local(marbl_tracer_indices%total_cnt, km)
    real(r8),                      intent(in)    :: poly_remin(km)
    real(r8),                      intent(in)    :: lipid_remin(km)
    ...
    associate(&
      ...
      do k=1,km
        ...
        denitrif(k) = work * ((POC_remin(k) * (c1 - POCremin_refract) &
                 - other_remin(k)) / denitrif_C_N  - sed_denitrif(k) &
                 + (poly_remin(k) / parm_Den_poly_C_NO3) &
                 + (lipid_remin(k) / parm_Den_lipid_C_NO3) &
                 + (DOCr_remin(k) / parm_Den_rDOM_C_NO3))
! Below is what has worked
!        denitrif(k) = work * ((poly_remin(k) + lipid_remin(k) + DOCr_remin(k) + POC_remin(k) * (c1 - POCremin_refract) &
!                 - other_remin(k)) / denitrif_C_N  - sed_denitrif(k))
! Below is something I coded
!        denitrif(k) = work * ((poly_remin(k) / parm_Den_poly_C_NO3) &
!                           +  (lipid_remin(k) / parm_Den_lipid_C_NO3) &
!                           +  (DOCr_remin(k) / parm_Den_rDOM_C_NO3) &
!                           +  (POC_remin(k) * (c1 - POCremin_refract) / denitrif_C_N) &
!		           -  (other_remin(k) / denitrif_C_N) &                  
!                           -   sed_denitrif(k))
  ...
  subroutine compute_local_tendencies(km, marbl_tracer_indices, autotroph_derived_terms, &
    ...
    associate(                                                            &
         ...
         auto_loss_poly  => autotroph_derived_terms%auto_loss_poly(:,:),  & ! auto_loss routed to poly (mmol C/m^3/sec)
         auto_loss_lipid  => autotroph_derived_terms%auto_loss_lipid(:,:),            & ! auto_loss routed to lipid (mmol C/m^3/sec)
         auto_loss_amino_poly  => autotroph_derived_terms%auto_loss_amino_poly(:,:),  & ! auto_loss routed to amino_poly (mmol C/m^3/sec)
         auto_loss_protein  => autotroph_derived_terms%auto_loss_protein(:,:),        & ! auto_loss routed to protein (mmol C/m^3/sec)
         ...
         auto_graze_poly  => autotroph_derived_terms%auto_graze_poly(:,:),   & ! auto_graze routed to poly (mmol C/m^3/sec)
         auto_graze_lipid => autotroph_derived_terms%auto_graze_lipid(:,:),  & ! auto_graze routed to lipid (mmol C/m^3/sec)
         auto_graze_amino_poly  => autotroph_derived_terms%auto_graze_amino_poly(:,:),  & ! auto_graze routed to amino_poly (mmol C/m^3/sec)
         auto_graze_protein     => autotroph_derived_terms%auto_graze_protein(:,:),     & ! auto_graze routed to protein (mmol C/m^3/sec)
         ...
         zoo_graze_poly    => zooplankton_derived_terms%zoo_graze_poly(:,:),  & ! grazing of zooplankton routed to poly (mmol C/m^3/sec)
         zoo_graze_lipid   => zooplankton_derived_terms%zoo_graze_lipid(:,:), & ! grazing of zooplankton routed to lipid (mmol C/m^3/sec)
         zoo_graze_amino_poly   => zooplankton_derived_terms%zoo_graze_amino_poly(:,:), & ! grazing of zooplankton routed to amino_poly (mmol C/m^3/sec)
         zoo_graze_protein      => zooplankton_derived_terms%zoo_graze_protein(:,:),    & ! grazing of zooplankton routed to protein (mmol C/m^3/sec)
         ...
         zoo_loss_poly   => zooplankton_derived_terms%zoo_loss_poly(:,:),   & ! zoo_loss routed to poly (mmol C/m^3/sec)
         zoo_loss_lipid  => zooplankton_derived_terms%zoo_loss_lipid(:,:),  & ! zoo_loss routed to lipid (mmol C/m^3/sec)
         zoo_loss_amino_poly => zooplankton_derived_terms%zoo_loss_amino_poly(:,:),  & ! zoo_loss routed to amino_poly (mmol C/m^3/sec)
         zoo_loss_protein    => zooplankton_derived_terms%zoo_loss_protein(:,:),     & ! zoo_loss routed to protein (mmol C/m^3/sec)

         DOC_prod        => dissolved_organic_matter%DOC_prod(:),          & ! production of DOC (mmol C/m^3/sec)
	 poly_prod       => dissolved_organic_matter%poly_prod(:),         & ! production of poly
	 lipid_prod      => dissolved_organic_matter%lipid_prod(:),        & ! production of lipid
         poly_remin      => dissolved_organic_matter%poly_remin(:),        & ! remineralization of poly
         lipid_remin     => dissolved_organic_matter%lipid_remin(:),       & ! remineralization of lipid
         DOCr_remin      => dissolved_organic_matter%DOCr_remin(:),        & ! remineralization of DOCr
	 amino_poly_prod => dissolved_organic_matter%amino_poly_prod(:),   & ! production of amino_poly
	 protein_prod    => dissolved_organic_matter%protein_prod(:),      & ! production of protein
         amino_poly_remin => dissolved_organic_matter%amino_poly_remin(:), & ! remineralization of amino_poly
         protein_remin   => dissolved_organic_matter%protein_remin(:),     & ! remineralization of protein
      ...
      do k=1, km
        ...
        interior_tendencies(nh4_ind,k) = -sum(NH4_V(:,k)) - nitrif(k) + amino_poly_remin(k) + protein_remin(k) + DONr_remin(k)  &
...
!       interior_tendencies(poly_ind,k) = poly_prod(k) - poly_remin(k)
!       interior_tendencies(poly_ind,k) = (poly_prod(k) * (c1 - (0.67_r8 * DOCprod_refract))) - poly_remin(k)
!       interior_tendencies(poly_ind,k) = (poly_prod(k) * (0.67_r8 * DOCprod_refract)) - poly_remin(k)
!       interior_tendencies(poly_ind,k) = (poly_prod(k) * (0.67_r8 * (c1 - DOCprod_refract))) - poly_remin(k)
       interior_tendencies(poly_ind,k) = (poly_prod(k) * (c1 - DOCprod_refract)) - poly_remin(k)

!       interior_tendencies(lipid_ind,k) = lipid_prod(k) - lipid_remin(k)
!       interior_tendencies(lipid_ind,k) = (lipid_prod(k) * (c1 - (0.33_r8 * DOCprod_refract))) - lipid_remin(k)
!       interior_tendencies(lipid_ind,k) = (lipid_prod(k) * (0.33_r8 * DOCprod_refract)) - lipid_remin(k)
!       interior_tendencies(lipid_ind,k) = (lipid_prod(k) * (0.33_r8 * (c1 - DOCprod_refract))) - lipid_remin(k)
       interior_tendencies(lipid_ind,k) = (lipid_prod(k) * (c1 - DOCprod_refract)) - lipid_remin(k)

        interior_tendencies(docr_ind,k) = DOC_prod(k) * DOCprod_refract - DOCr_remin(k) + (POC_remin(k) * POCremin_refract)

!        interior_tendencies(amino_poly_ind,k) = amino_poly_prod(k) - amino_poly_remin(k)
!        interior_tendencies(amino_poly_ind,k) = (amino_poly_prod(k) * (c1 - (0.6_r8 * DONprod_refract))) - amino_poly_remin(k)
!        interior_tendencies(amino_poly_ind,k) = (amino_poly_prod(k) * (0.6_r8 * DONprod_refract)) - amino_poly_remin(k)
!        interior_tendencies(amino_poly_ind,k) = (amino_poly_prod(k) * (0.6_r8 * (c1 - DONprod_refract))) - amino_poly_remin(k)
        interior_tendencies(amino_poly_ind,k) = (amino_poly_prod(k) * (c1 - DONprod_refract)) - amino_poly_remin(k)

!        interior_tendencies(protein_ind,k) = protein_prod(k) - protein_remin(k)
!        interior_tendencies(protein_ind,k) = (protein_prod(k) * (c1 - (0.4_r8 * DONprod_refract))) - protein_remin(k)
!        interior_tendencies(protein_ind,k) = (protein_prod(k) * (0.4_r8 * DONprod_refract)) - protein_remin(k)
!        interior_tendencies(protein_ind,k) = (protein_prod(k) * (0.4_r8 * (c1 - DONprod_refract))) - protein_remin(k)
        interior_tendencies(protein_ind,k) = (protein_prod(k) * (c1 - DONprod_refract)) - protein_remin(k)

        interior_tendencies(donr_ind,k) = (Q * f_toDON * DOC_prod(k) * DONprod_refract) - DONr_remin(k) &
        ...
        interior_tendencies(dic_ind,k) = sum(auto_loss_dic(:,k)) + sum(auto_graze_dic(:,k)) - sum(photoC(:,k)) &
                                       + poly_remin(k) + lipid_remin(k) + POC_remin(k) * (c1 - POCremin_refract) + sum(zoo_loss_dic(:,k)) &
        ...
        o2_consumption(k) = o2_consumption(k) * ((POC_remin(k) * (c1 - POCremin_refract) &
                                                  ...
                                                  + (poly_remin(k) / parm_Remin_poly_C_O2) &
                                                  + (lipid_remin(k) / parm_Remin_lipid_C_O2) &
                                                  + (DOCr_remin(k) / parm_Remin_rDOM_C_O2))
! This one below is what has worked
!        o2_consumption(k) = o2_consumption(k) * ((POC_remin(k) * (c1 - POCremin_refract) + poly_remin(k) + lipid_remin(k) + DOCr_remin(k) &
!                                                  - (sed_denitrif(k) * denitrif_C_N) - other_remin(k) &
!                                                  + sum(zoo_loss_dic(:,k)) + sum(zoo_graze_dic(:,k))  &
!                                                  + sum(auto_loss_dic(:,k)) + sum(auto_graze_dic(:,k))) &
!                                                 / parm_Remin_D_C_O2 + (c2 * nitrif(k)))
! This one below is something I coded
!        o2_consumption(k) = o2_consumption(k) * (((POC_remin(k) * (c1 - POCremin_refract)) / parm_Remin_D_C_O2) &
!                                              -  ((sed_denitrif(k) * denitrif_C_N) / parm_Remin_D_C_O2) &
!                                              -  (other_remin(k) / parm_Remin_D_C_O2) &
!                                              +  (sum(zoo_loss_dic(:,k)) + sum(zoo_graze_dic(:,k)) / parm_Remin_D_C_O2)  &
!                                              +  (sum(auto_loss_dic(:,k)) + sum(auto_graze_dic(:,k)) / parm_Remin_D_C_O2) &
!					      +  (DOCr_remin(k) / parm_Remin_rDOM_C_O2) &
!					      +  (poly_remin(k) / parm_Remin_poly_C_O2) &
!					      +  (lipid_remin(k) / parm_Remin_lipid_C_O2) &
!					      +  (c2 * nitrif(k)))
