...
contains
  ...
  subroutine marbl_diagnostics_init(  &
    ...
    associate(                                         &
      ...
      lname = 'Vertical Integral of Polysaccharides Production'
      sname = 'poly_prod_zint'
      units = 'mmol/m^3 cm/s'
      vgrid = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%poly_prod_zint, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname = 'Vertical Integral of Polysaccharides Production, 0-100m'
      sname = 'poly_prod_zint_100m'
      units = 'mmol/m^3 cm/s'
      vgrid = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%poly_prod_zint_100m, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname = 'Vertical Integral of Polysaccharides Remineralization'
      sname = 'poly_remin_zint'
      units = 'mmol/m^3 cm/s'
      vgrid = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%poly_remin_zint, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname = 'Vertical Integral of Lipids Remineralization'
      sname = 'lipid_remin_zint'
      units = 'mmol/m^3 cm/s'
      vgrid = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%lipid_remin_zint, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname = 'Vertical Integral of Polysaccharides Remineralization, 0-100m'
      sname = 'poly_remin_zint_100m'
      units = 'mmol/m^3 cm/s'
      vgrid = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%poly_remin_zint_100m, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname = 'Vertical Integral of Lipids Remineralization, 0-100m'
      sname = 'lipid_remin_zint_100m'
      units = 'mmol/m^3 cm/s'
      vgrid = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%lipid_remin_zint_100m, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if
      ...
      lname = 'Polysaccharides Production'
      sname = 'poly_prod'
      units = 'mmol/m^3/s'
      vgrid = 'layer_avg'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%poly_prod, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname = 'Lipids Production'
      sname = 'lipid_prod'
      units = 'mmol/m^3/s'
      vgrid = 'layer_avg'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%lipid_prod, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname = 'Polysaccharides Remineralization'
      sname = 'poly_remin'
      units = 'mmol/m^3/s'
      vgrid = 'layer_avg'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%poly_remin, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname = 'Lipids Remineralization'
      sname = 'lipid_remin'
      units = 'mmol/m^3/s'
      vgrid = 'layer_avg'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%lipid_remin, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if
      ...
      lname = 'Amino Polysaccharides Production'
      sname = 'amino_poly_prod'
      units = 'mmol/m^3/s'
      vgrid = 'layer_avg'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%amino_poly_prod, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname = 'Proteins Production'
      sname = 'protein_prod'
      units = 'mmol/m^3/s'
      vgrid = 'layer_avg'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%protein_prod, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname = 'Amino Polysaccharides Remineralization'
      sname = 'amino_poly_remin'
      units = 'mmol/m^3/s'
      vgrid = 'layer_avg'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%amino_poly_remin, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname = 'Proteins Remineralization'
      sname = 'protein_remin'
      units = 'mmol/m^3/s'
      vgrid = 'layer_avg'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%protein_remin, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if
  ...
  subroutine store_diagnostics_dissolved_organic_matter(marbl_domain, &
    ...
    diags(ind%poly_prod)%field_3d(:, 1)        = dissolved_organic_matter%poly_prod(:)
    diags(ind%lipid_prod)%field_3d(:, 1)       = dissolved_organic_matter%lipid_prod(:)
    diags(ind%poly_remin)%field_3d(:, 1)       = dissolved_organic_matter%poly_remin(:)
    diags(ind%lipid_remin)%field_3d(:, 1)      = dissolved_organic_matter%lipid_remin(:)
    diags(ind%DOCr_remin)%field_3d(:, 1)       = dissolved_organic_matter%DOCr_remin(:)
    diags(ind%amino_poly_prod)%field_3d(:, 1)  = dissolved_organic_matter%amino_poly_prod(:)
    diags(ind%protein_prod)%field_3d(:, 1)     = dissolved_organic_matter%protein_prod(:)
    diags(ind%amino_poly_remin)%field_3d(:, 1) = dissolved_organic_matter%amino_poly_remin(:)
    diags(ind%protein_remin)%field_3d(:, 1)    = dissolved_organic_matter%protein_remin(:)
    ...
    call marbl_diagnostics_share_compute_vertical_integrals(diags(ind%poly_remin)%field_3d(:,1), &
         delta_z, kmt, full_depth_integral=diags(ind%poly_remin_zint)%field_2d(1), &
         near_surface_integral=diags(ind%poly_remin_zint_100m)%field_2d(1))

    call marbl_diagnostics_share_compute_vertical_integrals(diags(ind%lipid_remin)%field_3d(:,1), &
         delta_z, kmt, full_depth_integral=diags(ind%lipid_remin_zint)%field_2d(1), &
         near_surface_integral=diags(ind%lipid_remin_zint_100m)%field_2d(1))
  ...
  subroutine store_diagnostics_carbon_fluxes(marbl_domain, POC, P_CaCO3, interior_tendencies, &
    ...
    ! vertical integrals
    work = interior_tendencies(dic_ind,:) + interior_tendencies(poly_ind,:) +            &
         interior_tendencies(lipid_ind,:) +                                              &
  ...
  subroutine store_diagnostics_nitrogen_fluxes(marbl_domain, &
    ...
    work = interior_tendencies(no3_ind,:) + interior_tendencies(nh4_ind,:) +                &
           interior_tendencies(amino_poly_ind,:) +                                          &
           interior_tendencies(protein_ind,:)    + interior_tendencies(donr_ind,:) +        &
